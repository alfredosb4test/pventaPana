...
if(array_key_exists("accion", $_POST) && $_POST['accion']=='order_prove'){
	$array_prove = $conn->listar_trasferencias($_POST["orderby"], $_SESSION['g_id_empresa'], $_SESSION['g_sucursales']);	
}else
	$array_prove = $conn->listar_trasferencias("ORDER BY id_trasferencias DESC", $_SESSION['g_id_empresa'], $_SESSION['g_sucursales']);	
...
if($array_prove == 'no_data'){
	...
	$tabla = '<table class="tbl_datos" border="1" cellpadding="2" cellspacing="0" width="100%">';
		$tabla .= "
		 	<tr class='f_negro table_top'>
				<th width='10%' id='th_ambito' class='hand t_hover_amarillo ordenar_query' campo=' ORDER BY nombre_empleado '><img src='images/sort.png' style='position:absolute;margin-top:2px;margin-left:-20px'>Empleado</th>
				<th width='9%' class='hand t_hover_amarillo ordenar_query' campo=' ORDER BY sucursal_origen '><img src='images/sort.png' style='position:absolute;margin-top:2px;margin-left:-20px'>Sucursal Origen</th>
				<th width='9%' class='hand t_hover_amarillo ordenar_query' campo=' ORDER BY sucursal_destino '><img src='images/sort.png' style='position:absolute;margin-top:2px;margin-left:-20px'>Sucursal Destino</th>
				<th width='32%' class='hand t_hover_amarillo ordenar_query' campo=' ORDER BY codigo '><img src='images/sort.png' style='position:absolute;margin-top:2px;margin-left:-20px'>Producto</th>
				<th width='5%' class='hand t_hover_amarillo ordenar_query' campo=' ORDER BY cant_trasf '><img src='images/sort.png' style='position:absolute;margin-top:2px;margin-left:-20px'>Cant.</th>
	...
	foreach($array_prove['id_trasferencias'] as $key=>$nombre){

		$tabla .= 
		'<tr class="tbl_fila tr_detalles" 
			id="'.$array_prove['id_trasferencias'][$key].'">
			<td>'.$array_prove['nombre_empleado'][$key].'</td>
			<td>'.$array_prove['sucursal_origen'][$key].'</td>
			<td>'.$array_prove['sucursal_destino'][$key].'</td>
			<td><span class="negritas">'.$array_prove['codigo'][$key].' </span> '.$array_prove['nombre_prod'][$key].'</td>
			<td align="center">'.$array_prove['cant_trasf'][$key].'</td>
...
?> 
  <div class="f_negro titulo_frm">
  	<div style="position:relative; top:7px;">Trasferencias</div>  
  </div> 
<?=$tabla;?>
